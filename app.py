...
def list_users():
    ...
    if not search:
        ...
        users = User.query.filter(User.username.ilike(f"%{search.strip()}%")).all()
...
def users_show(user_id):
    ...
    like_count=sum([len(msg.user_likes) for msg in user.messages])
...
def homepage():
    ...
    if g.user:
        ...
        messages=(Message.query.filter((Message.user_id.in_([i.id for i in g.user.following])) | (Message.user_id==g.user.id) )
                .order_by(Message.timestamp.desc())
                .limit(100)
                .all())
